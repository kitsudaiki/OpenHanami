...
impl PartialEq for TransferBlock {
...
mod tests {
    ...
    fn test_serialize_deserialize() {
        let original = TransferBlock::default();

        let cfg = bincode::config::standard();
        let serialized: Vec<u8> =
            bincode::serde::encode_to_vec(&original, cfg).expect("Failed to serialize");
        let deserialized: TransferBlock = bincode::serde::decode_from_slice(&serialized, cfg)
