...
template <class T>
class TemplateSocket : public AbstractSocket
{
   public:
    /**
     * @brief constructor
     ...
     */
    uint32_t getType() { return m_socket.type; }
     ...
     */
    bool isClientSide() { return m_socket.isClientSide(); }
    ...
    bool sendMessage(const void *message, const uint64_t numberOfBytes, ErrorContainer &error)
    {
        ...
        const ssize_t successfulSended
            = m_socket.sendData(m_socket.getSocketFd(), message, numberOfBytes, MSG_NOSIGNAL);
        m_lock.clear(std::memory_order_release);

        // check if the message was completely send
        if (successfulSended < -1 || successfulSended != static_cast<long>(numberOfBytes)) {
            return false;
        }
    ...
    bool closeSocket()
    {
        ...
        int32_t timeout = 100;
        while (m_isfullyClosed == false && timeout > 0) {
            usleep(100);
            timeout--;
        }
   ...
   private:
    ...
    bool waitForMessage()
    {
        ...
        const long recvSize = m_socket.recvData(
            m_socket.getSocketFd(), &m_recvBuffer.data[writePosition], spaceToEnd, 0);
